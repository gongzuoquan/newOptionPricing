					    ...
					    const unsigned int seed)
{
	...
	curand_init(seed+tid,0,0,&rngStates[tid]);
	return ;
}
...
void PricingEngine<Real>::operator()(AsianOption<Real> &option)
{
	...
	{
		cudaSetDevice(d);
		initRNG<<<grid[d],block[d],0,stream[d]>>>(d_rngStates[d],m_seed+m_numSims*3);
	...
	{
		cudaSetDevice(d);
		//computeValue<<<grid[d],block[d],2048*sizeof(Real),stream[d]>>>(d_values[d],d_valueSquares[d],d_paths[d],d_option[d],numTasks[d],numTimesteps);
